...
class Cheats:
	...
	def parse_yaml(self, filename):
		...
		for cheat in self.cheatlist:
			...
			# if the users provided multiple commands
			if type(cheat.command) == str:
				cheat.printable_command = cheat.command.replace('\\\n', '')
			elif type(cheat.command) == list:
				# only keep the first, other are exemples
				cheat.printable_command = cheat.command[0].replace('\\\n', '')
